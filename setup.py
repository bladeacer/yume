...
def _analyze_image_for_clustering(image_path, images_dir, num_colors):
    ...
    This is now a helper for the main clustering function.
    """
    try:
        from PIL import Image
        from collections import Counter
...
def _plot_cluster_graph(all_colors_array, cluster_centers, labels, ax):
    ...
    # Use the cluster labels to assign a color to each point
    ...
    # Plot the cluster centers (larger, different markers)
    ...
    # Set axis limits to 0-255 for full RGB range
    ax.set_xlim(0, 255)
    ax.set_ylim(0, 255)
    ax.set_zlim(0, 255)
...
def _show_color_gui(all_colors_data, all_grouped_colors, cluster_centers, labels, gui_max_swatches):
    ...
    # New call to plot all clustered data
...
def main():
    ...
    parser.add_argument(
        ...
        type=int,
        default=10,
        help="Number of top most frequent colors to display on the RGB graph (default: 10)."
    )
    parser.add_argument(
        "--num-clusters",
        type=int,
